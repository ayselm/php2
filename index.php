...
<body>
    ...
    function getFullnameFromParts($f, $i, $o)
    {
        // $fio = implode(' ', $arr);
        return  $f . ' ' . $i . ' ' . $o;
    }
    ...
    function getGenderFromName($fio)
    {
        ...
        if (endsWith($divide['name'], 'а')) {
            $gender--;
        } elseif (endsWith($divide['name'], 'й') || endsWith($divide['name'], 'н')) {
            $gender++;
        }

        if (endsWith($divide['surname'], 'ва')) {
            $gender--;
        } elseif (endsWith($divide['surname'], 'в')) {
            $gender++;
        }
    ...
    function  getGenderDescription($arr)
    {
        ...
        echo 'woman: ';
        print_r(round(100 * $woman / count($arr), 2));
        echo '<br>';
        echo 'man: ';
        print_r(round(100 * $man / count($arr), 2));
        echo '<br>';
        echo 'underfined: ';
        print_r(round(100 * $undf / count($arr), 2));
    }
    ...
    function getPerfectPartner($f, $i, $o, $arr)
    {
        ...
        $fio = $f . ' ' . $i . ' ' . $o;
        if (getGenderFromName($fio) != 'undefined') {
            do {
            ...
            echo ('<br>' . $fio . '  ' . getGenderFromName($fio) . ' + ' . $arr[$n]['fullname']);

            echo ('Идеально на ' .  round(rand(5000, 10000) / 100, 2) . '%');
        } else {
            echo '<br>для ' . $fio . ' пол=undefined: пару не подобрать';
        }
    }
    ...
    <p>array -> string: <?php print_r(getFullnameFromParts('пупкина', 'мария', 'хорезми'))  ?></p>
    ...
    getPerfectPartner('пупкина', 'мария', 'хорезми', $example_persons_array)
    ?>
</body>
...
>>>>>>> b2a9579b62461bac332017c70ae24e866db9b744
</html>